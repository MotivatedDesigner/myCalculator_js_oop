export default class Controller{
  ...
  fillPopup = (from) => {
    let html
    if(from === 'memory')
      html = this.memory.getAll().map( (el, i) => 
      ...
      html = this.history.getAll().map( (el, i) => 
      `<div class="memory-item hover2" >
        <h1>${el}</h1>
        <div class="memory-control" data-index="${i}">
          <input class="pink" type="button" value="MC">
